...
const MyTextArea = ({ label, actions, ...props }) => {
    const [field, meta] = useField(props);
    return (
        <div className='flex flex-col w-full'>
        <div className='flex flex-col '>
          ...
          <textarea
            className='outline-none text-mono-100 text-sm font-normal bg-mono-900 p-2 rounded h-32 border-2 border-solid border-mono-800 focus:border-primary-200 active:border-primary-200 active:bg-mono-800'
            {...field}
            {...props}
          />
        </div>
...
const FeedbackForm = () => {
    ...
    return (
        ...
        <Formik
            ...
            validationSchema={
                Yup.object({
                    submittedHeader: Yup.string().min(2).max(100).required("Required"),
                    submittedMessage: Yup.string().min(2).max(500).required("Required"),
            })}
            onSubmit={ async (values, { setSubmitting, resetForm }) => {

                const apiReq = await fetch("/api/feedback/", {
                    method: "POST",
                    headers: {
          ...
          <Form>
            <div className='gap-4 min-h-fit flex flex-col items-center justify-center'>      
                <div className=' w-full flex  flex-col md:flex-row justify-between items-center gap-1 md:gap-4'>
                    <MyTextInput
                    label='Title'
                    name='submittedHeader'
                    type='text'
                    placeholder="Can't submit bet"
                    />
                    <MyTextArea
                    label='Put your thoughts into words'
                    name='submittedMessage'
                    type='text'
                    placeholder='I tried to submit my bet but...'
                    />
                </div>               
                <button
                    type='submit'
