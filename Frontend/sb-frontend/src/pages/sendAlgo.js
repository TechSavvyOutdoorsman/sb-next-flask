...
const AlgoForm = () => {
    ...
    return (
        ...
        <Formik
            ...
            validationSchema={
                Yup.object({
                    submittedReceiver: Yup.string().min(58, 'Too Short!').max(58, 'Too Long!').required("Required"),
                    submittedQuantity: Yup.number().required("Required"),
            })}
            onSubmit={ async (values, { setSubmitting, resetForm }) => {

                const apiReq = await fetch("/api/send/", {
                    method: "POST",
                    headers: {
